# Chihyeon Yun | AI Researcher

<p align="center">
  <img src="https://github.com/ChihyeonYoon.png" width="150" style="border-radius: 50%; border: 3px solid #3498db;">
</p>

<p align="center">
  <a href="https://chihyeonyoon.github.io/">
    <img src="https://img.shields.io/badge/Portfolio-28CF75?style=flat-square&logo=googlechrome&logoColor=white"/>
  </a>
  <a href="https://github.com/ChihyeonYoon">
    <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=GitHub&logoColor=white"/>
  </a>
  <a href="mailto:chichi8969@naver.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=Gmail&logoColor=white"/>
  </a>
</p>

---

### 🌐 Personal Links
You can view my resume and portfolio online via the links below.
> **[👉 Chihyeon Yun github.io](https://chihyeonyoon.github.io/)**

---

### 🧑‍🔬 Research & Career
* **ETRI (Electronics and Telecommunications Research Institute)** | Post-Master's Researcher (Feb 2025 - Jun 2025)
    * Developed Visual Positioning System (VPS) for cyber-physical space registration.
    * Researched VLM-based anomaly detection systems for participants in metaverse platforms.
* **Kyungpook National University** | M.S. in Artificial Intelligence (Mar 2023 - Feb 2025)
    * **Multimodal Language & Cognition Lab**: Conducted research on deep learning-based automated multi-channel video editing, multimodal analysis, speech recognition, and sound analysis/generation.

---

### 📚 Publications

#### 1. IEEE Transactions on Industrial Informatics (In Revision)
**"Seamless Multi-Hypothesis Video Editing Using Deep Multimodal Analysis Over Multi-Channel Sources"**
* **Core Research**: A system that automatically generates professional-level video edits by analyzing visual and audio data collected from multi-camera environments.
* **Key Technologies**: Implemented natural camera transitions using Speaker State Estimation, Multi-Hypothesis Generation, and Beam Search Optimization.

#### 2. Scientific Data (2024)
**"A Military Audio Dataset for Situational Awareness and Surveillance"**
* **Core Research**: Constructed a high-quality audio dataset for situational awareness in specialized environments (military/surveillance) and contributed to domain-specific research by releasing it as an open dataset. (Scientific Data 11.1 (2024): 668)
* [Scientific Data](https://www.nature.com/articles/s41597-024-03511-w) | [Dataset](https://www.kaggle.com/datasets/junewookim/mad-dataset-military-audio-dataset)

#### 3. NeurIPS Workshop (2023)
**"Adversarial Fine-tuning using Generated Respiratory Sound to Address Class Imbalance"**
* **Core Research**: Proposed an Adversarial Fine-tuning technique to overcome data imbalance in the Medical AI domain by utilizing data generated through an Audio Diffusion Neural Vocoder. (NeurIPS Workshop on Deep Generative Models for Health 2023)
* [Overview Link](https://chihyeonyoon.github.io/Adversarial-Fine-tuning-using-Generated-Respiratory-Sound-to-Address-Class-Imbalance/) | [Github](https://github.com/ChihyeonYoon/Adversarial-Fine-tuning-using-Generated-Respiratory-Sound-to-Address-Class-Imbalance)
* [arXiv](https://arxiv.org/abs/2311.06480) | [Conference](https://openreview.net/forum?id=z1AVG5LDQ7)

#### 4. KSC 2023 Proceedings (2023)
**"CutMix-SupCon: Supervised Contrastive Learning with CutMix Augmentation for Multimodal-based Emotion Recognition"**
* **Core Research**: Enhanced the efficiency of multimodal emotion recognition by combining Feature-level CutMix augmentation with Supervised Contrastive Learning (SupCon) to address data imbalance. (KSC 2023 Proceedings: 2112-2114)
* [DBpia](https://www.dbpia.co.kr/journal/articleDetail?nodeId=NODE11488648&buildDate=2026-02-24+10%3A04%3A31&nowDate=20260303_1&cdnUrl=https%3A%2F%2Fcdn.dbpia.co.kr%2Fstatic&buildTime=20260224100431&minify=.min&appVersion=1.0.0&nodeId=NODE11488648&mobileYN=N&medaTypeCode=185005&isPDFSizeAllowed=true&locale=ko&foreignIpYn=N&articleTitle=CutMix-SupCon%3A+CutMix+%EC%A6%9D%EA%B0%95%EA%B3%BC+%EC%A7%80%EB%8F%84+%EB%8C%80%EC%A1%B0+%ED%95%99%EC%8A%B5%EC%9D%84+%ED%99%9C%EC%9A%A9%ED%95%9C+%EB%A9%80%ED%8B%B0%EB%AA%A8%EB%8B%AC+%EA%B8%B0%EB%B0%98+%EA%B0%90%EC%A0%95%EC%9D%B8%EC%8B%9D&articleTitleEn=CutMix-SupCon%3A+Supervised+Contrastive+Learning+with+CutMix+Augmentation+for+Multimodal-based+Emotion+Recognition&voisId=VOIS00726737&voisName=%ED%95%9C%EA%B5%AD%EC%A0%95%EB%B3%B4%EA%B3%BC%ED%95%99%ED%9A%8C+2023+%ED%95%9C%EA%B5%AD%EC%BB%B4%ED%93%A8%ED%84%B0%EC%A2%85%ED%95%A9%ED%95%99%EC%88%A0%EB%8C%80%ED%9A%8C+%EB%85%BC%EB%AC%B8%EC%A7%91&voisCnt=732&searchResultKey=%2FlMfWK6MWZi5N74t6Fqz8UQcUOwv%2FslS4mPcqI4dGZgk8ljuhcW5xDnI5QeCG48V&language=ko_KR&hasTopBanner=true)

---

### 📩 Contact
* **Email**: [chichi8969@naver.com](mailto:chichi8969@naver.com)
* **GitHub**: [@ChihyeonYoon](https://github.com/ChihyeonYoon)

---
<p align="center">© 2026 Chihyeon Yun. All rights reserved.</p>

